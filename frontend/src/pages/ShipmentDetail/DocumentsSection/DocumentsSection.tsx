...
const DocumentsSection: React.FC<DocumentsSectionProps> = ({
  ...
  // Fetch the documents attached to this shipment (again on retry)
  useEffect(() => {
    let cancelled = false;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      clearInterval(progressInterval);
      // Keep the selected file so the user can retry the upload.
      setUploadProgress(0);
      setIsUploading(false);
  ...
  return (
    ...
    >
      ...
      {isLoadingDocs ? (
        ...
        <div
          role="alert"
          className="flex flex-col items-center justify-center py-12 text-[rgba(255,255,255,0.6)] gap-3 text-center"
        >
          <p className="m-0 text-sm">Couldn&apos;t load documents for this shipment.</p>
          <button
            type="button"
            onClick={() => setReloadKey((key) => key + 1)}
            className="text-[#00d4c8] text-sm font-medium bg-[rgba(0,212,200,0.08)] border border-[rgba(0,212,200,0.2)] rounded-lg px-4 py-2 transition-all hover:bg-[rgba(0,212,200,0.15)] hover:border-[rgba(0,212,200,0.4)]"
          >
            Retry
          </button>
        </div>
